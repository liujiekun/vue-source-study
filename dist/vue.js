...
(function (global, factory) {
  ...
  function makeMap (
    ...
    return expectsLowerCase
      ...
      : function (val) { return map[val]; } // 根据传入的str生成的map，并返回一个(val)=>map[val]，以确定val是否在生成的map中
  }
   ...
   */
  function toObject (arr) { // 浅复制将数组扩展成对象
    var res = {};
  ...
  methodsToPatch.forEach(function (method) {
    ...
    def(arrayMethods, method, function mutator() {
      ...
      var result = original.apply(this, args); //还调用原型的方法
      var ob = this.__ob__;
      var inserted;
      switch (method) {
        case 'push':
  ...
  var Observer = function Observer(value) {
    ...
    if (Array.isArray(value)) {
      if (hasProto) {
        // 这个就厉害了直接给对象设置原型target.__proto__ = src
        protoAugment(value, arrayMethods);
      } else {
        // 如果不支持原型方式，那就挨个添加属性，确保数组调用方法时，调用的是改造过的方法
        copyAugment(value, arrayMethods, arrayKeys);
      }
      this.observeArray(value); // value是数组的话，observeArray，循环observe
    } else {
      this.walk(value); // value是对象,直接defineReactive属性，说白了数组循环observe最终都是走到这里，实现的defineReactive
    }
  };
  ...
  function defineReactive$$1 (
    ...
    var dep = new Dep(); // 定义变量的dep

    var property = Object.getOwnPropertyDescriptor(obj, key);
    if (property && property.configurable === false) {
      return // 如果不可配置，就不用往下走了，下面就是配置的
    }
    ...
    var childOb = !shallow && observe(val); // 对象的属性也是对象，递归
    Object.defineProperty(obj, key, {
      enumerable: true,
      configurable: true,
  ...
  function normalizeProps(options, vm) {
    ...
    if (Array.isArray(props)) {
      ...
      for (var key in props) {
        val = props[key];
        name = camelize(key);
        res[name] = isPlainObject(val) ? val : { type: val }; // 这算限制了个啥类型嘛，props:{'XXX':'type'}, 默认当作类型限制
      }
    } else {
      warn(
    ...
    if (Array.isArray(inject)) {
      ...
      for (var key in inject) {
        ...
        normalized[key] = isPlainObject(val)
          ? extend({ from: key }, val)
          : { from: val };
        // 写法：inject:{key:{from:'xxx', default:'yyy'}}
        // 写法：inject:{key:value},默认作为from
      }
    } else {
      warn(
  ...
  function validateProp(
    ...
    var prop = propOptions[key]; //组件里要的值
    var absent = !hasOwn(propsData, key); // 不在它父占位组件的属性里面
    var value = propsData[key]; // 先取父占位组件里面传过来的值
    // boolean casting
    var booleanIndex = getTypeIndex(Boolean, prop.type); //有匹配到是Boolean类型
    if (booleanIndex > -1) {
  ...
  function assertProp(
    ...
    if (type) {
      ...
      for (var i = 0; i < type.length && !valid; i++) {
        var assertedType = assertType(value, type[i]); // 通过value和设定的type进行比对
...

  {
    ...
    var hasHandler = {
      has: function has (target, key) {
        var has = key in target; // Vue本身的属性
  ...
  var normalizeEvent = cached(
    function (
      ...
      var passive = name.charAt(0) === "&"; // passive的快捷符号
      name = passive ? name.slice(1) : name;
      var once$$1 = name.charAt(0) === "~"; // Prefixed last, checked first，// once的快捷符号
      name = once$$1 ? name.slice(1) : name;
      var capture = name.charAt(0) === "!"; // 捕获阶段的快捷符号
  ...
  function extractPropsFromVNodeData(
    ...
    if (isDef(attrs) || isDef(props)) {
      // 组件定义的props
      for (var key in propOptions) { // 组件里定义的
        // 接收的属性，转换成kebab-case
        var altKey = hyphenate(key); // 把驼峰liuJieKun转成->liu-jie-kun
        {
  ...
  function checkProp(
    ...
    altKey,
    preserve
  ) {//如果被作为props识别了，就从$attrs中删去,$attrs只保留除了props之外的属性
    if (isDef(hash)) {
  ...
  function normalizeScopedSlots (
    ...
    if (!slots) {
      ...
      res = {};
      for (var key$1 in slots) { // slot中有scope的节点
        if (slots[key$1] && key$1[0] !== '$') {//过滤掉$打头的属性
    ...
    for (var key$2 in normalSlots) {// slot中没有scope的节点
      if (!(key$2 in res)) {
  ...
  function renderSlot (
    ...
    var scopedSlotFn = this.$scopedSlots[name]; // scope="scope" slot-scope="scope"
    var nodes;
    ...
    var target = props && props.slot; // 如果slot-scope 的东西，有属性slot然后就
    if (target) {
  ...
  function createComponent(
    ...
    var baseCtor = context.$options._base; // 其实就是Vue

    // plain options object: turn it into a constructor
    if (isObject(Ctor)) {
      Ctor = baseCtor.extend(Ctor); // 瞬间给子组件赋能,创建子组件构造函数
    }

    // if at this stage it's not a constructor or an async component factory,
    // reject.
    // 刚开始不是，但现在是了，由对象转成的function
    ...
    resolveConstructorOptions(Ctor); // 修正一下options以及superOptions，extendOptions
    ...
    // 给data.hook={}增加一下钩子:[init,prepatch,insert,destroy]
    ...
    var vnode = new VNode(
      ...
      { Ctor: Ctor, propsData: propsData, listeners: listeners, tag: tag, children: children }, // 这个就是所谓的componentOption
      asyncFactory
    );
  ...
  function renderMixin (Vue) {
    ...
    Vue.prototype._render = function () {
      ...
      try {
        ...
        vnode = render.call(vm._renderProxy, vm.$createElement); // 如果客户自己写的render，vm.$createElement其实就是render的参数(h)
      } catch (e) {
  ...
  function initEvents(vm) {
    ...
    var listeners = vm.$options._parentListeners; // 是它爸爸的监听事件吗？
    if (listeners) {
  ...
  function lifecycleMixin (Vue) {
    ...
    Vue.prototype._update = function (vnode, hydrating) {
      var vm = this;
      var prevEl = vm.$el; // 第一次是undefined
      var prevVnode = vm._vnode; // 更新前的vnode,第一次也是undefined
  ...
  function mountComponent (
    ...
    new Watcher(vm, updateComponent, noop, {
      before: function before () { // 渲染watcher独有的触发beforeUpdate钩子
  ...
  */
  ...
  function initProps(vm, propsOptions) {
    ...
    var keys = (vm.$options._propKeys = []); // 存放porps的key
    ...
    var loop = function ( key ) {
      keys.push(key);
      var value = validateProp(key, propsOptions, propsData, vm); // 校验类型，并observe值，返回值
      /* istanbul ignore else */
      {
        var hyphenatedKey = hyphenate(key);
        if (
  ...
  var computedWatcherOptions = { lazy: true }; // computed数据的watcher标识
  ...
  function initInternalComponent(
    ...
    var parentVnode = options._parentVnode; // 占位vnode
    opts.parent = options.parent; // 父组件所在的实例，好像是一个组件一个实例
    opts._parentVnode = parentVnode;

    var vnodeComponentOptions = parentVnode.componentOptions; // { Ctor, propsData, listeners, tag, children }
  ...
  function resolveConstructorOptions(Ctor) {
    // 构造器就是Vue
    var options = Ctor.options; //现在的Vue.options {comonents:{keep-alive,transition,transition-group},filters:{},directives:{v-model,v-show},_base=Vue}
    if (Ctor.super) {
      // 什么样的会有super? 答：父子组件extend之后，子组件就有了
      var superOptions = resolveConstructorOptions(Ctor.super); // 如果他有super会继续递归，superOptions是新读取到的父组件构造函数的的options
      var cachedSuperOptions = Ctor.superOptions; // 这是子组件生成时记住的父组件构造函数的options
  ...
  function resolveModifiedOptions(Ctor) {
    var modified;
    var latest = Ctor.options; // superOptions
    var sealed = Ctor.sealedOptions; // mergeOptions{superOptions,Sub.options}
  ...
  function initExtend(Vue) {
    ...
    Vue.extend = function (extendOptions) {
      ...
      var Super = this; // 就是Vue
      var SuperId = Super.cid; // 初始值是0被Vue占了，以后的组件从1开始++
      ...
      if (Sub.options.props) {
        initProps$1(Sub); // 初始化子组件props
      }
      if (Sub.options.computed) {
        initComputed$1(Sub); // 初始化子组件computed
      }
  ...
  function initGlobalAPI (Vue) {
    ...
    initMixin$1(Vue); // Vue.mixin,$data,$props,$set,$delete,$watch
  ...
  function createPatchFunction (backend) {
    var i, j;
    var cbs = {};
    // 刚装进去，有解构出来，显得你能吗？净瞎折腾
    var modules = backend.modules;
    var nodeOps = backend.nodeOps;
    // 现在的modules:[
    //   directives:{create,update,destroy},
    //   ref:{create,update,destroy},
    //   attrs:{create,update},
    //   klass:{create,update},
    //   domPops:{create,update},
    //   events:{create,update},
    //   style:{create,update},
    //   transition:{create,update,remove}
    // ]
    ...
    //   destroy:[directive.destroy,ref.destroy]
    // }
    ...
    // 组件渲染参数：undefined,vnode,false,    false,也就是说组件的$el是undefined
    // 初次渲染参数：vm.$el,   vnode,false,    false
    // 渲染更新参数：oldVnode, vnode,undefined,undefined
    ...
    if (val.indexOf('[') < 0 || val.lastIndexOf(']') < len - 1) {
      ...
      if (index$1 > -1) {
        ...
        return {
          ...
          key: '"' + val.slice(index$1 + 1) + '"', //{exp:A,key:'B'} || {exp:A['B'],key:C}
        };
      } else { // 没有.说明只绑定了某个值
        return {
          exp: val,
  ...
  function parseHTML(html, options) {
    ...
    function parseStartTag() {
      ...
      if (start) {
        ...
        if (end) {
          match.unarySlash = end[1]; // 是否是单闭合标签
    ...
    function handleStartTag(match) {
      ...
      for (var i = 0; i < l; i++) {
        ...
        // 正则表达式匹配结果，args[1]是name,2是等号，3是双引号中间值，4是单引号中间值，5是直接写值
  ...
  function parse (
      ...
      if (currentParent && !element.forbidden) {
        if (element.elseif || element.else) {
          ...
          if (element.slotScope) { // 有slotScope的，都是<template scope="">或者slot-scope的
            ...
            var name = element.slotTarget || '"default"' // 有slot的就放到对应的slot里面，如果没有放default
    ...
    parseHTML(template, {
      ...
      start: function start (tag, attrs, unary, start$1, end) {
        ...
        {
          if (options.outputSourceRange) { // 浏览器环境为真
        ...
        if (!inVPre) {
          processPre(element); // 如果有v-pre就不编译了好像
          if (element.pre) {
            inVPre = true;
          }
        }
        if (platformIsPreTag(element.tag)) { // 直接pre标签
          inPre = true;
        }
        if (inVPre) {
        ...
        if (!unary) { // 如果不是自闭合标签，就赋值当前父元素，并将元素入栈
          ...
          // 如果是单闭合标签直接就处理了，否则等到end的closeElement来处理
  ...
  function parseFor(exp) {
    ...
    var alias = inMatch[1].trim().replace(stripParensRE, ""); // 把表达式左右括号去掉,变成item,index
    var iteratorMatch = alias.match(forIteratorRE); // 剩下了item,index 用来匹配都好后面的\,index
    if (iteratorMatch) {
  ...
  function processAttrs (el) {
    ...
    for (i = 0, l = list.length; i < l; i++) {
      ...
      if (dirRE.test(name)) {// /^v-|^@|^:|^\.|^#/ 或者/^v-|^@|^:|^#/
        ...
        if (modifiers) {
          // 本来属性名是@native.keypress，现在name = @native
  ...
  function generate (
    ...
    // 初始化state，里面有个directives,合并了baseDirectives和options.directives
    // baseDirectives:{on:function,bind:function,cloak:noop}
    // options.directives:{html:function,model:function,text:function}
  ...
  function genFor (
    ...
    var exp = el.for;
    var alias = el.alias;
    var iterator1 = el.iterator1 ? ("," + (el.iterator1)) : ''; // 加上逗号当参数了
    var iterator2 = el.iterator2 ? ("," + (el.iterator2)) : ''; // 加上逗号当参数了
  ...
  function genSlot (el, state) {
    ...
    var res = "_t(" + slotName + (children ? ("," + children) : ''); // _t:renderSlot
  ...
  Vue.prototype.$mount = function (
    ...
    if (!options.render) {
      var template = options.template; // 先找options里面的template
      if (template) {
